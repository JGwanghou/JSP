...
<%
	...
	// 글 가져오기
	BoardBean board = dao.selectArticle(no);
	
	// 댓글 가져오기
	List<BoardBean> comments = dao.selectComments(no);
%>
...
<main id="board">
    <section class="view">
        <table border="0">
            ...
            <tr>
                <th>제목</th>
                <td><input type="text" name="title" readonly value="<%= board.getTitle() %>"/></td>
            </tr>
            
            <% if(board.getFile() > 0){ %>
            <tr>
                <th>파일</th>
                <td><a href="/Jboard1/proc/download.jsp?parent=<%= board.getNo() %>"><%= board.getOriName() %></a>&nbsp;<span><%= board.getDownload() %></span>회 다운로드</td>
            </tr>
            <% } %>
            <tr>
                <th>내용</th>
                <td>
                    <textarea name="content" readonly><%= board.getContent() %></textarea>
                </td>
            </tr>                    
        </table>
        
        <div>
        	<%if(ub.getUid().equals(board.getUid())){ %>
            <a href="/Jboard1/proc/deleteProc.jsp?no=<%= board.getNo() %>&pg=<%= pg %>" class="btn btnRemove">삭제</a>
            <a href="/Jboard1/modify.jsp?no=<%= board.getNo() %>&pg=<%= pg %>" class="btn btnModify">수정</a>
            <% } %>
        ...
        <section class="commentList">
            ...
            <% for(BoardBean comment : comments) { %>                 
            <article>
